...
exports.mediafireDl = async function (url) {
  try {
     const {data} = await axios.get(`https://www-mediafire-com.translate.goog/${url.replace('https://www.mediafire.com/','')}?_x_tr_sl=en&_x_tr_tl=fr&_x_tr_hl=en&_x_tr_pto=wapp`)
     ...
     const result = { scraper: 'by: @ianvanh', name, size, date, mime, link }
     return({status: true, ...result})
  } catch {
    return({status: false, creator: '@ianvanh', message: 'error'})
  } 
}
...
exports.igstalk = async function (Username) {
...
exports.tanggal = (numer) => {
  ...
  const time = moment.tz(global.timeZone).format("DD/MM HH:mm:ss");
  let d = new Date();
  let locale = "co";
  let gmt = new Date(0).getTime() - new Date("1 Enero 1980").getTime();
