...
exports.getTime = (format, date) => {
	if (date) {
		return moment(date).locale('co').format(format)
	} else {
		return moment.tz('America/Bogota').locale('co').format(format)
	}
}

exports.formatDate = (n, locale = 'co') => {
	let d = new Date(n)
...
fs.watchFile(file, () => {
	fs.unwatchFile(file)
	log(pint(`Update ${__filename}`, 'red.'))
