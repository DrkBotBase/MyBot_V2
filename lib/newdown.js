...
let quality = $('div').find('#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(3) > a').attr('data-fquality')
let tipe = $('div').find('#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(3) > a').attr('data-ftype')
let output = `${title}.` + tipe
let size = $('div').find('#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(2)').text()
let sizeKB = parseFloat(size) * (1000 * /MB$/.test(size))
...
hasil.push({ thumb, title, quality, tipe, size, sizeKB, output, link })
return hasil[0]
}
